        ...
        {featuredAssets.map((item) => {
          ...
          return (
            <a
              ...
              {...externalLinkProps}
              className={[
                'group h-full min-h-[260px] rounded-2xl border p-6 shadow-sm backdrop-blur transition-all duration-300 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300',
                item.highlighted
            ...
            >
              <article className="flex h-full flex-col">
                ...
                <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-slate-300">理由：{item.reason}</p>
                <p className="mt-3 text-xs text-slate-400">{item.useCount}</p>
                <span className="mt-auto inline-flex w-full items-center justify-center gap-1 rounded-xl bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-500 px-4 py-2.5 text-sm font-semibold text-white transition-all duration-300 group-hover:brightness-110">
